...
def find_discord(relative_err, timestamps, threshold=0.01):
    ...
    return relative_err[-1], timestamps[-1]

def process_data(exp_path, threshold=0.01):
    rewards_agent_e = np.load(exp_path + "rewards_agent_e.npy")
    relative_err = np.load(exp_path + "relative_err_hist.npy")
    values = np.load(exp_path + "repr_values.npy")
    Vpi = np.load(exp_path + "Vpi_agent_e.npy")
...
def main():
    ...
    for n_iter in n_iters:
        for seed in seeds:
            exp_path = f"./saved_values{n_iter}_{seed}/"
            # if n_iter == 2500:
            #     threshold = 0.015
            # else:
            #     threshold = 0.01
